...
export const activeMadLibQuery = createEntityQuery<HistoryMadLib>(
  activeMadLibStore
);

export const getActiveMadLib$ = activeMadLibQuery.select();

export const activeMadLibControls$ = activeMadLibQuery.select(
  (state) => state.controls
);

export const activeMadLibCategories$ = activeMadLibQuery.select(
